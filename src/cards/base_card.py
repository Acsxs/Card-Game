...
class Card(EventBroadcaster):
    ...
    def apply_card_effects(self):
        ...
        if (self.calculate('shield') != 0) and (self.calculate('heal') != 0):
            self.broadcast(Event(f"CardDefend{target}", (self.calculate('shield'), self.calculate('heal'), self.target)))

        if len(self.calculate('effects')) != 0:
            self.broadcast(Event(f"CardEffect{target}", (self.calculate('effects'), self.target)))

        if len(self.calculate('meta')) != 0:
            self.broadcast(Event("CardMetaEffectCard", self.calculate('meta')))
